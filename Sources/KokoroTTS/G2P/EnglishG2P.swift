...
final class EnglishG2P {
    ...
    static let punctuactions: Set<Character> = Set(";:,.!?\u{2014}\u{2026}\u{201C}\u{201D}\u{201E}")
    ...
    static let nonQuotePunctuations: Set<Character> = Set(
        punctuactions.filter { !"\u{201C}\u{201D}\u{201E}".contains($0) })
                    ...
                    })
                {
                    if let val = EnglishG2P.punctuationTagPhonemes[token.text] {
                        ...
                        token.phonemes = token.text.filter {
                            EnglishG2P.punctuactions.contains($0)
                        }
                    }
                    token.`_`.rating = 4
